...
- Add a `View` menu to the Log View toolbar, with two options that persist across reloads. `Highlight commits on '<branch>'` dims every commit that is not on the current branch, so what the branch you are on actually contains stands out; it is greyed out when HEAD is detached, since there would be no branch to compare against. `No merge commits` drops merges from the log, including one reached by pasting its hash.
- Remove the divider lines between the branch, commit and details panes, so the three read as one surface. The drag handle still occupies the same 6px and only shows itself when you are on it, so resizing is unchanged.
