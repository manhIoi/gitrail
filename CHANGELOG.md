...
- Add `Highlight my commits`, on by default, as `giPro.logView.highlightMyCommits`. Commits you authored show their name in bold and in a deep sea blue — `#0b5a8a` in the light theme, `#74bdea` in the dark one — leaving the row background to the branch highlight so both can be read at once. The blue is held well away from the graph's `gc-7` lane, which can be drawn two columns away in the same row, and carries 7.38:1 and 8.01:1 against the row background, 6.40:1 and 7.00:1 on a highlighted row. Matched on `git config user.name`, the same way the User filter's `Me` entry already does, so the two can never disagree. The option is greyed out when no user name is set.
...
- Stop a branch that is behind turning its whole row one colour. The branch glyph, the arrow and the count were all painted with the same orange, so a row that was merely out of date read as a warning and the glyph stopped saying "branch" at all. The glyph now keeps its blue in every tracking state, the way IntelliJ's branch popup does, and only the small direction arrow carries the state — `↓` in coral when behind, `↑` in blue when ahead.
- Give the ahead/behind count its own blue rather than leaving it to the glyph's colour, so it reads as a count you can click toward rather than as part of the branch name — which is what IntelliJ does. `#6089ef` in the dark theme, and `#4069e0` in the light one: IntelliJ's own `#4573e8` two steps darker, since at 12px bold its 4.31:1 sits just under the ratio text wants on white.
- Lift the light theme's branch icon colours, which had been darkened past the point where they still read as the colour they were meant to be. The amber shared by a branch that is behind and by the current branch was `#8a6000` and `#a06000` — yellow at 27% lightness, which reads as olive mud rather than amber — and is now a single `#b45309`, ten points lighter and rotated away from the olive band, still clear of the red that marks a deleted file. The branch icon blue is lifted at the same hue, `#0066bb` to `#0072d1`. Every one of these stays above the 4.5:1 contrast ratio on white. The dark theme is untouched; its equivalents were already bright.
